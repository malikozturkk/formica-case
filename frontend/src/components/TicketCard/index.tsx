...
const TicketCard: React.FC<ITicketCard> = ({ travel }) => {
    const citiesCol = "text-xs md:text-sm text-[#8392a7] font-semibold block truncate whitespace-nowrap"
    const dateCol = "text-[10px] md:text-sm text-[#8392a7] font-semibold block"
    const hourCol = "flex flex-col items-center relative text-xs md:text-2xl font-semibold text-[#444763]"
    return (
        <div className="rounded-3xl bg-white overflow-hidden relative flex flex-col bg-clip-border break-words" style={{ boxShadow: "0 10px 24px 0 rgba(68,60,98,.22)" }}>
            <div className="border-b-0 bg-white rounded-3xl cursor-pointer p-3 md:py-4 md:px-8">
            <button className="text-2xl font-semibold text-[#444763] p-0 relative cursor-pointer text-left align-middle select-none bg-transparent border border-transparent leading-6 rounded-sm w-full">
                <div className="flex flex-nowrap w-full items-center justify-center">
                    ...
                    <div className="max-w-fit md:max-w-1/4 md:px-4">
                        <div className="md:pr-5">
                        <span className="text-[10px] md:text-sm font-bold text-[#8392a7]">Bilet Ücreti</span>
                        <p className="text-base md:text-3xl font-bold text-[#444763] my-3 leading-normal">₺780,00</p>
                        </div>
                    </div>
                </div>
            </button>
