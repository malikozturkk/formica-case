    ...
    return (
        <div className="w-full rounded-3xl flex flex-col gap-8 p-3 md:px-5 md:py-6" style={{ boxShadow: "0 10px 24px 0 rgba(68,60,98,.22)" }}>
            ...
            {!ticketsData[0]?.ticketNumber || ticketsData.length === 0 ? 
                ...
                return (
                    <div key={ticket.ticketNumber} className={`rounded-3xl bg-white overflow-hidden relative flex flex-col bg-clip-border break-words ${status === "USED" || status === "EXPIRED" ? "opacity-50 cursor-not-allowed" : "opacity-100 cursor-pointer"}`} style={{ boxShadow: "0 10px 24px 0 rgba(68,60,98,.22)" }}>
                            ...
                            >
                                <div className="flex flex-nowrap w-full items-center justify-center">
                                    <div className="flex flex-col gap-1 max-w-fit md:max-w-1/4 w-full items-center md:items-start">
                                        ...
                                        <div className="text-xs md:text-sm font-bold px-1 py-0.5 w-fit rounded-md border border-gray-300">{statusText[status]}</div>
